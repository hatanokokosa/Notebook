...
export async function getBlogData({ locale }: StarlightRouteData, t: App.Locals["t"]): Promise<StarlightBlogData> {
  ...
  const blogData: StarlightBlogData = { posts };
...
async function getBlogSidebar(context: APIContext): Promise<StarlightRouteData["sidebar"]> {
      ...
      makeSidebarGroup(
        ...
        [...tags]
          ...
          .map(([tagSlug, { entries, label }]) =>
            makeSidebarLink(`${label} (${entries.length})`, getRelativeBlogUrl(`/tags/${tagSlug}`, locale), isBlogTagPage(id, tagSlug)),
          ),
      ),
    );
  }
  ...
  if (context.site && config.rss) {
    sidebar.push(makeSidebarLink(t("kokosaBlog.sidebar.rss"), getRelativeBlogUrl("/rss.xml", locale, true), false));
  }
