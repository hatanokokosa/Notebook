...
// Expose the kokosa-blog plugin configuration and project context.
export function vitePluginKokosaBlogConfig(starlightBlogConfig: StarlightBlogConfig, context: StarlightBlogContext): VitePlugin {
  const modules = {
    "virtual:kokosa-blog/config": `export default ${JSON.stringify(starlightBlogConfig)}`,
    "virtual:kokosa-blog/context": `export default ${JSON.stringify(context)}`,
  };
