...
export function getRelativeBlogUrl(path: string, locale: Locale, ignoreTrailingSlash = false) {
  ...
  return getRelativeUrl(getPathWithLocale(path ? `/${config.prefix}/${path}` : `/${config.prefix}`, locale), ignoreTrailingSlash);
}
...
export function getSidebarProps(slug: string, entries: StarlightBlogEntry[], locale: Locale): StarlightRouteData["sidebar"] {
