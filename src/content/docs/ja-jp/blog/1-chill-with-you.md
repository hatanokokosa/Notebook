...
tags:
  - Gaming
  - Proton
excerpt: Steam に「Chill with You ~ Lofi Story」というゲーム（ゲームか？実質ポモドーロタイマー！）がある。結構気に入っているゲームで、ヒロインが可愛くて、一緒にいてくれる感がすごく強い。設定上はヒロインと自分が別々のパラレルワールドにいることになっているので、ゲーム内に表示される時間と自分のローカル時間に時差があるのも、ずっとその設定の一部だと思って放置していた……
---
...
- `TZDIR=/etc/zoneinfo`：タイムゾーンファイルのディレクトリを指定
...
この起動オプションを追加してからゲームを起動すると、時刻が正常に表示されるようになった。9:31 はちゃんと 9:31 に。
...
最後に：この記事以降、ファイル名の先頭に番号をつけるようにしました。じゃないと散らかってしまうので。
